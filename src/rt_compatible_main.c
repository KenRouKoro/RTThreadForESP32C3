...
systimer_ll_clear_alarm_int(1);
    ...
    /* leave interrupt */
    rt_interrupt_leave();
     systimer_ll_is_alarm_int_fired( 1);
}

void main_thread_entry(void)
{
    printf("RTThread Nano for ESP32C3 \n");
        uint32_t count = 0;
        uint8_t system_timer_counter=1;

    //rt_hw_interrupt_enable(0);
    esp_intr_alloc(ETS_SYSTIMER_TARGET1_EDGE_INTR_SOURCE, ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_LEVEL1, t_SysTickIsrHandler, NULL, NULL);
    systimer_hal_init();
    systimer_ll_set_counter_value( system_timer_counter, 0);
    systimer_ll_apply_counter_value( system_timer_counter);
    uint32_t alarm_id = 1 ;
    systimer_hal_connect_alarm_counter( alarm_id, system_timer_counter);
    systimer_hal_set_alarm_period( alarm_id, 1000000UL / CONFIG_FREERTOS_HZ);
    systimer_hal_select_alarm_mode( alarm_id, SYSTIMER_ALARM_MODE_PERIOD);
        systimer_hal_counter_can_stall_by_cpu(1, 0, true);
    systimer_hal_enable_alarm_int( alarm_id);
